...
instruction ins[N];
ll n,num_load,num_store,num_add,num_mult,cnt,time_load,time_store,time_add,time_mult,time_div,CLOCK=1;
ll board[N][5],history_board[MAX_CLOCK][N][5]; 
string load_status[10][10],store_status[10][10],add_status[10][10],mult_status[10][10],register_results[33];
string history_load_status[MAX_CLOCK][10][10],history_store_status[MAX_CLOCK][10][10],history_add_status[MAX_CLOCK][10][10],history_mult_status[MAX_CLOCK][10][10],history_register_results[MAX_CLOCK][33];
string f_register_name[33], r_register_name[33];
vector<ReservationStation> add_rs,mult_rs,load_buffers,store_buffers;
...
void init() 
{
    ...
    store_buffers.resize(num_store+1);
...
bool issue_instruction(ll ins_id) 
{
    ...
    { 
        for(ll i=1;i<=num_store;i++) 
        {
            ...
            {
                store_buffers[i].Busy=true;
                store_buffers[i].Op="STORE";
                store_buffers[i].ins_id=ins_id;
                store_buffers[i].ready_cycle=-1;
                store_buffers[i].time_remaining=time_store;
                if(F_reg_status[inst.fj].Qi==-1) 
                {
                    store_buffers[i].Vj=F_reg_values[inst.fj];
                    store_buffers[i].Qj=-1;
                } 
                else
                {
                    store_buffers[i].Qj=F_reg_status[inst.fj].Qi;
                    if(F_reg_status[inst.fj].Qi<=num_load)   store_buffers[i].Qj_source="Load"+to_string(F_reg_status[inst.fj].Qi);
                    else if(F_reg_status[inst.fj].Qi<=num_load+num_add) store_buffers[i].Qj_source="Add"+to_string(F_reg_status[inst.fj].Qi-num_load);
                    else store_buffers[i].Qj_source="Mult"+to_string(F_reg_status[inst.fj].Qi-num_load-num_add);
                }
                if(F_reg_status[inst.fk].Qi==-1) 
                {
                    store_buffers[i].Vk=F_reg_values[inst.fk];
                    store_buffers[i].Qk=-1;
                } 
                else
                {
                    store_buffers[i].Qk=F_reg_status[inst.fk].Qi;
                    if(F_reg_status[inst.fk].Qi<=num_load)   store_buffers[i].Qk_source="Load"+to_string(F_reg_status[inst.fk].Qi);
                    else if(F_reg_status[inst.fk].Qi<=num_load+num_add) store_buffers[i].Qk_source="Add"+to_string(F_reg_status[inst.fk].Qi-num_load);
                    else store_buffers[i].Qk_source="Mult"+to_string(F_reg_status[inst.fk].Qi-num_load-num_add);
                }
                store_buffers[i].A=inst.fi;
                inst.issue=CLOCK;
                board[ins_id][1]=CLOCK;
                inst.issued=true;
                store_status[i][0]="Yes";
                store_status[i][1]=to_string(inst.fi)+"(R"+to_string(inst.fk)+")";
                if(store_buffers[i].Qj==-1)
                    store_status[i][2]=to_string(store_buffers[i].Vj);
                else
                    store_status[i][2]=store_buffers[i].Qj_source;
                    
                cout<<"Cycle "<<CLOCK<<": Issued STORE ins_id="<<ins_id<<", buffer="<<i<<", Vj="<<store_buffers[i].Vj<<", Qj="<<store_buffers[i].Qj<<", Vk="<<store_buffers[i].Vk<<", Qk="<<store_buffers[i].Qk<<", A="<<store_buffers[i].A<<"\n";
                return 1;
            }
        }
    } 
...
void execute() 
{
    ...
    for(ll i=1;i<=num_store;i++) 
    {
    	if(CLOCK==board[store_buffers[i].ins_id][1]) continue;
        if(store_buffers[i].Busy&&store_buffers[i].Qk==-1&&store_buffers[i].time_remaining>0) 
        {
            store_buffers[i].time_remaining--;
            if(store_buffers[i].time_remaining==0)
            {
                ll address=(ll)(store_buffers[i].Vk+store_buffers[i].A);
                store_buffers[i].result=store_buffers[i].Vj;
                ll ins_id=store_buffers[i].ins_id;
                ins[ins_id].exec=CLOCK;
                board[ins_id][2]=CLOCK;
                ins[ins_id].executed=true;
                store_status[i][3]="M"+to_string(i);
                cout<<"Cycle "<<CLOCK<<": STORE buffer "<<i<<" completed, ins_id="<<ins_id;
                cout<<", address="<<address<<", value="<<store_buffers[i].Vj<<"\n";
            } 
            else  cout<<"Cycle "<<CLOCK<<": STORE buffer "<<i<<" executing, time_remaining="<<store_buffers[i].time_remaining<<"\n";
        }
    }
    for(ll i=1;i<=num_add;i++) 
    {
...
void write_back() 
{
    for(ll i=1;i<=num_load;i++) 
    {
        ...
        if(load_buffers[i].Busy&&load_buffers[i].time_remaining==0&&ins[load_buffers[i].ins_id].executed&&!cdb_busy) 
        {
            ...
            for(ll j=1;j<=num_store;j++) 
            {
                if(store_buffers[j].Qj==i) 
                {
                    store_buffers[j].Vj=result;
                    store_buffers[j].Qj=-1;
                    store_status[j][2]=to_string(result);
                    cout<<"Cycle "<<CLOCK<<": Updated STORE buffer "<<j<<" Qj with "<<result<<"\n";
                }
            }
    ...
    for(ll i=1;i<=num_store;i++) 
    {
    	if(CLOCK==board[store_buffers[i].ins_id][2]) continue;
        if(store_buffers[i].Busy&&store_buffers[i].time_remaining==0&&ins[store_buffers[i].ins_id].executed&&!cdb_busy) 
        {
            double value=store_buffers[i].Vj;
            ll address=(ll)(store_buffers[i].Vk+store_buffers[i].A);
            ll ins_id=store_buffers[i].ins_id;
            ...
            board[ins_id][3]=CLOCK;
            ins[ins_id].written=true;
            store_buffers[i].Busy=false;
            cdb_busy=true;
            store_status[i][0]="No";
            store_status[i][1]="";
            store_status[i][2]="";
            cout<<"Cycle "<<CLOCK<<": Wrote back STORE buffer "<<i<<", ins_id="<<ins_id<<", stored "<<value<<" to address "<<address<<"\n";
            return;
        }
    }
    for(ll i=1;i<=num_add;i++) 
    {
        ...
        if(add_rs[i].Busy&&add_rs[i].time_remaining==0&&ins[add_rs[i].ins_id].executed&&!cdb_busy) 
        {
            ...
            for(ll j=1;j<=num_store;j++) 
            {
                if(store_buffers[j].Qj==i+num_load) 
                {
                    store_buffers[j].Vj=result;
                    store_buffers[j].Qj=-1;
                    store_status[j][2]=to_string(result);
    ...
    for(ll i=1;i<=num_mult;i++) 
    {
        ...
        if(mult_rs[i].Busy&&mult_rs[i].time_remaining==0&&ins[mult_rs[i].ins_id].executed&&!cdb_busy) 
        {
            ...
            for(ll j=1;j<=num_store;j++) 
            {
                if(store_buffers[j].Qj==i+num_load+num_add) 
                {
                    store_buffers[j].Vj=result;
                    store_buffers[j].Qj=-1;
                    store_status[j][2]=to_string(result);
...
void print_board() 
{
    ...
    for(ll i=1;i<=cnt;i++) 
    {
		...
		if(ins[i].opt==1||ins[i].opt==2)  cout<<left<<setw(13)<<("F"+to_string(ins[i].fi)+", "+to_string(ins[i].fj)+"(R"+to_string(ins[i].fk)+") ");
...
void print_store_status() 
{
    cout<<"----------------------------\n";
    cout<<"|     Store Status         |\n";
    cout<<"|Store  Busy    Address    |\n";
    for(ll i=1;i<=num_store;i++) 
	{
		string name="store"+to_string(i);
        cout<<"|"<<left<<setw(6)<<name<<" "<<setw(5)<<left<<store_status[i][0]<<"    "<<setw(10)<<store_status[i][1]<<"|\n";
    }
   cout<<"----------------------------\n";
}
...
void update_history(ll tt)
{
    ...
    for(ll i=0;i<=9;i++)
    {
        for(ll j=0;j<=2;j++) history_store_status[tt][i][j]=store_status[i][j];
    }
...
void write_history_back(ll tt)
{
    ...
    for(ll i=0;i<=9;i++)
    {
        for(ll j=0;j<=2;j++) store_status[i][j]=history_store_status[tt][i][j];
    }
...
int main() 
{
	...
	if(input_opt==1)
	{
	    cout<<"指令数 n （请输入数字）：";
		cin>>n;
		cout<<"LOAD 部件数量 num_load （请输入数字）：";
		cin>>num_load;
		cout<<"STORE部件数量 num_store（请输入数字）：";
		cin>>num_store;
		cout<<"ADD  部件数量 num_add  （请输入数字）：";
		cin>>num_add;
		cout<<"MULT 部件数量 num_mult （请输入数字）：";
		cin>>num_mult;
		cout<<"LOAD   指令延迟时间 time_load （请输入数字）：";
		cin>>time_load; 
		cout<<"STORE  指令延迟时间 time_store（请输入数字）：";
		cin>>time_store; 
		cout<<"ADD/SUB指令延迟时间 time_add  （请输入数字）：";
		cin>>time_add; 
	...
	else if(input_opt==2)  cin>>n>>num_load>>num_store>>num_add>>num_mult>>time_load>>time_store>>time_add>>time_mult>>time_div;
