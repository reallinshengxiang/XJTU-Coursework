...
using namespace std;
#define ll long long
ll n,num_load,num_add,num_mult,time_load,time_store,time_add,time_sub,time_mult,time_div;
int main() 
{
    srand(time(0));
    n=rand()%10+1;
    ...
    ll total_weight=num_load+num_add+num_mult;
    if(total_weight == 0) 
	{
        total_weight=1;
        if(num_load==0) 
	...
	cout<<num_load<<" "<<num_add<<" "<<num_mult<<"\n"<<time_load<<" "<<time_add<<" "<<time_mult<<" "<<time_div<<"\n";
    for(ll i=0;i<n;i++) 
	{
        string instr;
