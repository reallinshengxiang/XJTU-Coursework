...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for v in eliminationOrder:
            FactorsList, joinFactor = joinFactorsByVariable(FactorsList, v)
            if len(joinFactor.unconditionedVariables()) > 1:
                FactorsList.append(eliminate(joinFactor, v))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for i in range(len(self.items())):
            item1.append(items[i][1])
        return random.choices(list(self.keys()), weights=item1)[0]
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for i in self.allPositions:
            self.beliefs[i] = self.beliefs[i] * self.getObservationProb(observation, gameState.getPacmanPosition(), i, self.getJailPosition())
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            old_belief = self.beliefs[oldPos]
            for newPos, P in newPosDist.items():
                new_beliefs[newPos] = new_beliefs[newPos] + old_belief * P
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        BeliefDistribution = DiscreteDistribution()
        for particle in self.particles:
            BeliefDistribution[particle] = BeliefDistribution[particle] + self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition())
        BeliefDistribution.normalize()
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(ghostPositions[i % len(ghostPositions)])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                weight = weight * self.getObservationProb(observation[i], gameState.getPacmanPosition(), particle[i], self.getJailPosition(i))
            BeliefDistribution[particle] = BeliefDistribution[particle] + weight
        BeliefDistribution.normalize()
        if BeliefDistribution.total() == 0:
            self.initializeUniformly(gameState)
            BeliefDistribution = self.getBeliefDistribution()
