...
hp a[83],pow2[83],ans,dp[83][83],maxx,xx=2;
ll n,m;
int main()
{
	...
	cin>>n>>m;
	pow2[0]=1;
	for(ll i=1;i<=m;i++) pow2[i]=pow2[i-1]*2;
	for(ll ii=1;ii<=n;ii++)
	{
		...
		for(ll i=1;i<=m;i++)
		{
			for(ll j=m;j>=i;j--) dp[i][j]=max(dp[i][j],max(dp[i][j+1]+pow2[m-j+i-1]*a[j+1],dp[i-1][j]+pow2[m-j+i-1]*a[i-1]));
		}
		for(ll i=1;i<=m;i++) maxx=max(maxx,dp[i][i]+pow2[m]*a[i]);
		ans=ans+maxx;
	}
	cout<<ans;	
	return 0;
