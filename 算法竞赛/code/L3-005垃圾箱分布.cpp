...
{
	if(x.mindis>y.mindis) return 1;
	if(x.mindis==y.mindis&&x.ave<y.ave) return 1;
	if(x.mindis==y.mindis&&x.ave==y.ave&&x.id<y.id) return 1;
	return 0;
}
void dijkstra(ll st)
{
...
int main()
{
	...
	cout<<"G"<<ans[1].id<<"\n"<<fixed<<setprecision(1)<<ans[1].mindis<<" "<<ans[1].ave;
	return 0;
}
