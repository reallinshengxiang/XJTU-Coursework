...
bool lock_init(lock_t* lock) {
  ...
  return syscall1(SYS_LOCK_INIT, lock);
}

void lock_acquire(lock_t* lock) {
  int success = syscall1(SYS_LOCK_ACQUIRE, lock);
  if (success == 0) {
    exit(1);
  }
}
...
void lock_release(lock_t* lock) {
  int success = syscall1(SYS_LOCK_RELEASE, lock);
  if (success == 0)
    exit(1);
}
...
bool sema_init(sema_t* sema, int val) {
  ...
  return syscall2(SYS_SEMA_INIT, sema, val);
}
