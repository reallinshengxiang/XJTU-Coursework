...
void thread_tick(void) {
  ...
  if (active_sched_policy == SCHED_PRIO) {
    if (!list_empty(&priority_queue)) {
      struct thread* rdy = list_entry(list_front(&priority_queue), struct thread, elem);
      if (rdy->effective_priority > t->effective_priority) {
...
bool thread_priority_comparator(const struct list_elem* a, const struct list_elem* b,
                                void* aux UNUSED) {
...
static void thread_enqueue(struct thread* t) {
  ...
  if (active_sched_policy == SCHED_FIFO) {
    ...
    //priority_queue
    list_insert_ordered(&priority_queue, &t->elem, thread_priority_comparator, NULL);
...
void recurse_donate(struct thread* t) {
  struct thread* curr = t;
  while (curr->donated_to && curr->donated_to->holder) {
    struct thread* next = curr->donated_to->holder;
    if (next->effective_priority < curr->effective_priority) {
      next->effective_priority = curr->effective_priority;
      if (next->status == THREAD_READY) {
        list_remove(&next->elem);
        list_insert_ordered(&priority_queue, &next->elem, thread_priority_comparator, NULL);
      }
      curr = next;
    } else {
      break;
...
/* Updates effective priority of thread */
void update_priority(struct thread* t) {
  ...
  struct list_elem* e;
  for (e = list_begin(&t->donated_locks); e != list_end(&t->donated_locks); e = list_next(e)) {
    struct lock* l = list_entry(e, struct lock, elem);
    if (!list_empty(&l->semaphore.waiters)) {
      struct thread* highest_waiter =
          list_entry(list_front(&l->semaphore.waiters), struct thread, elem);
      if (highest_waiter->effective_priority > max_priority) {
        max_priority = highest_waiter->effective_priority;
      }
    }
  }
...
void thread_set_priority(int new_priority) {
  ...
  if (!list_empty(&priority_queue)) {
    struct thread* next = list_entry(list_front(&priority_queue), struct thread, elem);
    if (next->effective_priority > current->effective_priority) {
      thread_yield();
    }
  }
...
static struct thread* thread_schedule_prio(void) {
  if (!list_empty(&priority_queue))
    return list_entry(list_pop_front(&priority_queue), struct thread, elem);
  else {
    return idle_thread;
  }
}
