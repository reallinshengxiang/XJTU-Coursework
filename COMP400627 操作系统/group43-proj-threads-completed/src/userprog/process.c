...
bool setup_thread(void (**eip)(void), void** esp, void* thrd_);
...
void process_exit(void) {
  struct thread* tmain = thread_current()->pcb->main_thread;
  ...
  for (int i = 0; i < 256; i++) {
    struct lock* lock = &tmain->pcb->user_locks[i];
    if (lock_held_by_current_thread(lock)) {
      lock_release(lock);
    }
  }

  for (e = list_begin(&tmain->pcb->thread_joins); e != list_end(&tmain->pcb->thread_joins);
       e = list_next(e)) {
    struct join_elem* curr = list_entry(e, struct join_elem, elem);
    if (curr->tid == tmain->tid) {
      sema_up(&curr->join);
      break;
    }
  }

  tmain->pcb->marked_for_slaughter = true;

  for (e = list_begin(&tmain->pcb->thread_list); e != list_end(&tmain->pcb->thread_list);
       e = list_next(e)) {
    struct thread* curr = list_entry(e, struct thread_elem, elem)->thread;
    if (!is_main_thread(curr->tid, tmain->pcb)) {
      pthread_join(curr->tid);
    }
  }

  for (int i = 0; i < 256; i++) {
    if (tmain->pcb->user_locks[i] != NULL) {
      if (lock_held_by_current_thread(tmain->pcb->user_locks[i])) {
        lock_release(tmain->pcb->user_locks[i]);
      }
      free(tmain->pcb->user_locks[i]);
    }
  }

  for (int i = 0; i < 256; i++) {
    if (tmain->pcb->user_semas[i] != NULL) {
      free(tmain->pcb->user_semas[i]);
    }
  }

  void* upage = ((uint8_t*)PHYS_BASE) - PGSIZE * (tmain->page_no + 1);
  void* kpage = pagedir_get_page(tmain->pcb->pagedir, upage);
  pagedir_clear_page(tmain->pcb->pagedir, upage);
  palloc_free_page(kpage);

  lock_acquire(&tmain->pcb->pages->lock);
  tmain->pcb->pages->is_free[tmain->page_no] = true;
  lock_release(&tmain->pcb->pages->lock);

  free(tmain->pcb->pages);

  /* Close executable (and allow writes). */
  safe_file_close(tmain->pcb->bin_file);

  /* Free entries of children list. */
  for (e = list_begin(&tmain->pcb->children); e != list_end(&tmain->pcb->children); e = next) {
    struct wait_status* cs = list_entry(e, struct wait_status, elem);
    next = list_remove(e);
    release_child(cs);
  }
  ...
  while (!list_empty(&tmain->pcb->fds)) {
    e = list_begin(&tmain->pcb->fds);
  ...
  if (pd != NULL) {
    ...
    tmain->pcb->pagedir = NULL;
    pagedir_activate(NULL);
    pagedir_destroy(pd);
  }

  /* Notify parent that we're dead, as the last thing we do. */
  if (tmain->pcb->wait_status != NULL) {
    struct wait_status* cs = tmain->pcb->wait_status;
    printf("%s: exit(%d)\n", tmain->pcb->process_name, cs->exit_code);
    sema_up(&cs->dead);
    release_child(cs);
  }
  ...
  /* Free the PCB of this process and kill this thread
     ...
     If this happens, then an unfortuantely timed timer interrupt
     can try to activate the pagedir, but it is now freed memory */
  struct process* pcb_to_free = tmain->pcb;
  tmain->pcb = NULL;
...
bool setup_thread(void (**eip)(void), void** esp, void* thrd_) {
  ...
  *eip = (void (*)(void))(thrd->sf);
  if (push(kpage, &ofs, &thrd->arg, 4) == NULL || push(kpage, &ofs, &thrd->tf, 4) == NULL ||
      push(kpage, &ofs, &null, sizeof null) == NULL) {
    lock_acquire(&t->pcb->pages->lock);
    t->pcb->pages->is_free[t->page_no] = true;
    lock_release(&t->pcb->pages->lock);
  ...
  *esp = upage + ofs;
  ...
  //   memcpy(*esp - 4, &thrd->tf, 4);
  //   memcpy(*esp - 8, &thrd->arg, 4);
  //   memset(*esp - 12, 0, 4);
...
tid_t pthread_execute(stub_fun sf UNUSED, pthread_fun tf UNUSED, void* arg UNUSED) {
  ...
  char thread_name[16] = "shamith";
  tid_t tid;

  /* Initialize thrd_info. */
  exec.sf = sf;
  exec.tf = tf;
  exec.arg = arg;
  exec.pcb = t->pcb;
