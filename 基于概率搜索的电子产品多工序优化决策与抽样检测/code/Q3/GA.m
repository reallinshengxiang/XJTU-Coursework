...
plot(1:M, best_cost, 'b-o', 'LineWidth', 2, 'MarkerSize', 6, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'r');

% 设置坐标轴标签
xlabel('迭代次数 (M)', 'FontSize', 14, 'FontWeight', 'bold');
ylabel('最佳引理', 'FontSize', 14, 'FontWeight', 'bold');
...
% 设置图例位置和外观
legend('Best Cost', 'Location', 'Best', 'FontSize', 12, 'FontWeight', 'bold');

% 美化坐标轴
set(gca, 'FontSize', 12, 'LineWidth', 1.2, 'GridAlpha', 0.6, 'MinorGridAlpha', 0.3);

% 添加填充背景色和更大间隔的刻度
set(gca, 'Color', [0.95, 0.95, 0.95]);  % 背景色微灰
set(gca, 'XMinorTick', 'on', 'YMinorTick', 'on'); % 启用次刻度
