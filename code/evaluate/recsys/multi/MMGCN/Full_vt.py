...
def full_vt(epoch, model, data, prefix, writer=None):   
    ...
    with no_grad():
        precision, recall, ndcg_score, hr = model.full_accuracy(data)
        print('---------------------------------{0}-th Precition:{1:.4f} Recall:{2:.4f} NDCG:{3:.4f} HR:{4:.4f}---------------------------------'.format(
            epoch, precision, recall, ndcg_score, hr))
        if writer is not None:
            ...
            writer.add_scalar(prefix+'_HR', hr, epoch)
        ...
        return precision, recall, ndcg_score, hr
